$(document).ready(function () {
    ...
    if (getParameterByName("group") == "" || getParameterByName("group") == null) {
        ...
        KotakTaskboardNya(getParameterByName("group"));
        //getWS_DataTableTaskboard(getParameterByName("group"));
    }
        ...
        success: function (data) {
            ...
            if (json.length == 0) {
                ...
                for (i = 0; i < json.length; i++) {
                    ...
                    myTaskboardTicket.row.add([json[i].TicketNumber, json[i].NamePIC, json[i].CategoryName, json[i].SubCategory3Name, json[i].SLA, json[i].UsedDaySLAOK, TrxPosition, TrxParam, ConverTanggal, urlAction]).draw(false);
...
function KotakTaskboardNya(GroupAgent) {
    var ValUserID = $("#hd_sessionLogin").val();
    var ValLayerID = $("#TrxLoginTypeAngka").val();
    var result = "";
    var messageDiv = $('#2_TampungKotakAtas');
