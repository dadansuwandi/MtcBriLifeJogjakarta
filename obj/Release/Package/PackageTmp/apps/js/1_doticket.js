        ...
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                if (ParameterPage == "4") {
                    ...
                    var urlClick = " <a href='1_journey.aspx?ticketid=" + json[i].TicketNumber + "&numberid=" + ParameterNumberID + "&threadid=" + ParameterThreadID + "&channel=" + ParameterChannel + "&access=1&status=" + json[i].Status + "' class='text-success' data-toggle='tooltip' data-original-title='Follow up'><i class='ti-arrow-circle-right' aria-hidden='true'></i></a>"
                }
                if (json[i].Status == 'Open') {
                        ...
                        success: function (data) {
                            ...
                            for (i = 0; i < json.length; i++) {
                                ...
                                resultComposeBodyAttachment = '<ul class="mailbox-attachments clearfix">' +
                                    ...
                                    '<span class="mailbox-attachment-icon"><i class="fa fa-file-' + FileTypes + '-o"></i></span>' +
                                    '<div class="mailbox-attachment-info">' +
                                    '<a href=http://10.28.2.222/brilifecc/FileEmail/INBOX/' + json[i].EMAIL_ID + '/' + encodeData(json[i].FILENAME) + ' target="_blank" class="mailbox-attachment-name"><i class="fa fa-paperclip"></i>' + json[i].FILENAME + '</a>' +
                                    '<span class="mailbox-attachment-size">' + TrxFILESIZE + '' +
                                    '<a href=http://10.28.2.222/brilifecc/FileEmail/INBOX/' + json[i].EMAIL_ID + '/' + encodeData(json[i].FILENAME) + ' target="_blank" class="btn btn-default btn-xs pull-right"><i class="fa fa-cloud-download"></i></a>' +
                                    '</span>' +
                                    '</div>' +
                                    '</li>' +
                                    '<ul>'
            ...
            success: function (data) {
                ...
                for (i = 0; i < json.length; i++) {
                    ...
                    if (json[i].DIRECTION == "IN") {
                        var TrxFILENAME = '<a href=http://10.28.2.222/brilifecc/FILeEmail/INBOX/' + encodeURI(json[i].URL) + ' target="_blank" class="mailbox-attachment-name"><i class="fa fa-paperclip"></i>' + json[i].FILENAME.substring(0, 15) + '</a>'
                        var TrxDIRECTION = '<a href=http://10.28.2.222/brilifecc/FILeEmail/INBOX/' + encodeURI(json[i].URL) + ' target="_blank" class="btn btn -default btn - xs pull- right"><i class="fa fa - cloud - download"></i></a>'
                    } else if (json[i].DIRECTION == "OUT") {
                        var TrxFILENAME = '<a href=http://10.28.2.222/brilifecc/FILeEmail/OUTBOX/' + encodeURI(json[i].URL) + ' target="_blank" class="mailbox-attachment-name"><i class="fa fa-paperclip"></i>' + json[i].FILENAME.substring(0, 15) + '</a>'
                        var TrxDIRECTION = '<a href=http://10.28.2.222/brilifecc/FILeEmail/OUTBOX/' + jsonencodeURI(json[i].URL) + ' target="_blank" class="btn btn -default btn - xs pull- right"><i class="fa fa - cloud - download"></i></a>'
                    }
